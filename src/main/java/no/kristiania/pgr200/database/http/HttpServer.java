    ...
    private void serverThread(ServerSocket server) throws SQLException {
        while (true) {
            try {
                ...
                HttpPath httpPath = new HttpPath("/api/talks?status=200&body=title=test%26description=hello");
                //httpPath.getQuery();

                //System.out.println("/api/talks?status=200&body=title=test%26description=hello");
                //System.out.println(httpPath.getParameter("body"));

                //Prøv å flytte ut dette i en egen klasse Controllers
                switch (httpMethod) {
                    case "POST":
                ...
                //Response from server - Flytt til HttpResponse?
